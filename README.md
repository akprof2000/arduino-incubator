...
- [Железо и подключение](#железо-и-подключение)
  - [Полярность выходов](#полярность-выходов)
...
Силовая часть управляется **низким уровнем**: опторазвязанные модули реле
и силовые ключи включают нагрузку, когда на входе `LOW`. Светодиод и зуммер
аварии подключены к выводу напрямую и включаются высоким уровнем.

| Выход | Пин | Включено | Выключено |
|---|---|---|---|
| ТЭН нагрева (ШИМ) | 2 | `LOW` (скважность `255 − мощность`) | `HIGH` |
| Вентилятор обдува | 3 | `LOW` | `HIGH` |
| Испаритель | 4 | `LOW` | `HIGH` |
| Лоток: влево / вправо | 17 / 18 | `LOW` | `HIGH` |
| Светодиод аварии | 5 | `HIGH` | `LOW` |
| Зуммер аварии | 6 | `HIGH` | `LOW` |

Полярность задана **в одном месте** — [`Incubation/io.h`](Incubation/io.h),
константами `POWER_ACTIVE_LOW` и `ALARM_ACTIVE_LOW`. Код регуляторов не знает
про уровни и оперирует смыслом: `heaterPower(200)`, `coolerSet(true)`,
`trayMoveLeft()`, `evaporatorSet(false)`. Под другую схемотехнику достаточно
поменять две константы, а не два десятка вызовов `digitalWrite` в пяти файлах.

Таблица истинности проверяется компилятором на каждой сборке:

```cpp
static_assert(levelFor(true,  POWER_ACTIVE_LOW) == LOW,  "Нагрузка должна включаться низким уровнем");
static_assert(powerDuty(255,  POWER_ACTIVE_LOW) == 0,    "Полная мощность = постоянный LOW = ТЭН включён");
```

Отдельно решена проблема момента включения питания: при `active-LOW` обычная
последовательность `pinMode(OUTPUT)` → `digitalWrite(HIGH)` на короткое время
оставляет вывод в нуле, то есть **включает нагрузку**. Поэтому `initOutputSafe()`
сначала выставляет безопасный уровень (пока вывод ещё вход — это включает
подтяжку) и только потом переводит его в выход.
...
| `io.h` | полярность выходов и управление исполнительными устройствами |
...
- **Исправлена полярность силовых выходов.** Код включал нагрузку высоким
  уровнем, тогда как ключи и модули реле в приборе включаются низким —
  то есть ТЭН, обдув, испаритель и лоток работали инвертированно.
  Уровни вынесены в `io.h` и проверяются `static_assert` на каждой сборке.
  Заодно устранено кратковременное включение нагрузок в момент настройки
  выводов при подаче питания.
